...
class AccountMoveSS(models.Model):
    ...
    def _define_sale(self):
        for record in self:
            ...
            primarys_sale = self.env['purchase.order'].search([('tipificacion_compra.name','=','Productos Primarios')])
            offices_sale = self.env['purchase.order'].search([('tipificacion_compra.name','=','Productos de Oficina')])
            ben_diss_sale = self.env['purchase.order'].search([('tipificacion_compra.name','=','Descuentos y Beneficios')])
