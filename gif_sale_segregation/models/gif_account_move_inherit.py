    ...
    def _onchange_partner_id_changetype_sale(self):
        for record in self:
            try:
                ...
                res_as['domain'] = {'pay_type_sale': [('id', 'in',type_account_id_s)]}
                return res_as
            except:
                pass
        ...
        def onchange_type_of_product(self):
            for record in self:
                if record.move_id.type_of_purchase:
                    res = {}
                    temp = record.move_id.payment_type_purchase
