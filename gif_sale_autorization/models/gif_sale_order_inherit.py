...
class SaleOrder(models.Model):
    ...
    @api.onchange('pricelist_id','gif_sale_inverse_currency')
    def _onchange_pricelist_id_variable_exchange_sale(self):
        for record in self:
            try:
                ...
                record.gif_temp_sale = record.gif_sale_inverse_currency
    ...
    @api.onchange('gif_sale_inverse_currency')
...
class SaleOrderLine(models.Model):
    ...
    gif_difference = fields.Float(string='Cantidad',default=0.0)
    gif_is_different = fields.Boolean(string='Varia',default=False)
    ...
    def _onchange_product_template_id_reset_order_line_sa(self):
        for record in self:
            if record.product_template_id.name == False:
                gif_pasa = True
                pass
            else:
        ...
        for record in self:
            for partner_sale in record.product_template_id.partners_details:
                ...
                if record.price_unit < record.d_p_id_sales:
                    print('El precio de: ',record.gif_use_dif)
                    print('Es menor que: ', record.d_p_id_sales)
                    record.gif_is_different = True
                    if record.order_id.gif_temp_sale != 0:
                        print('Temp diferente de 0')
                    ...
                    if record.order_id.partner_id.name == partner_sale.partner.name:
                        if record.order_id.gif_temp_sale:
                            ...
                            if record.gif_use_dif < record.d_p_id_sales:
                                print('Es menor')
                                print('El virtual: ')
                                print(record.virtual_available_at_date)
                                record.gif_is_different = True
                                record.gif_difference = partner_sale.partner_price - record.price_unit
                            else:
                                record.gif_is_different = False
